...
static bool IsDlgTemplateExtended(DLGTEMPLATE *dlgTemplate) {
    ...
    // If signature is any value other than 0xFFFF, this is a standard dialog box template that uses
    // the DLGTEMPLATE and
...
static unsigned char definputbox_dlg[] = {
    };

static LPCSTR definputbox_buttonnames[] = {"OK", "CANCEL"};
static const INT_PTR definputbox_buttonids[] = {IDOK, IDCANCEL};

static const INT definputbox_id_prompt = 1000, definputbox_id_edit1 = 1001,
...
CWin32InputBox::CWin32InputBox(WIN32INPUTBOX_PARAM *param) { _param = param; }
CWin32InputBox::~CWin32InputBox() {}
void CWin32InputBox::SetParam(WIN32INPUTBOX_PARAM *param) { _param = param; }
WIN32INPUTBOX_PARAM *CWin32InputBox::GetParam() { return _param; }
...
INT_PTR CWin32InputBox::InputBoxEx(WIN32INPUTBOX_PARAM *param) {
    ...
    if (dlgTemplateEx != 0) {
        ...
        if (param->bCenter) dlgTemplateEx->style |= DS_CENTER;
        if (param->xPos != -1) dlgTemplateEx->x = param->xPos;
        if (param->yPos != -1) dlgTemplateEx->y = param->yPos;
    } else {
        ...
        if (param->bCenter) dlgTemplate->style |= DS_CENTER;
        if (param->xPos != -1) dlgTemplate->x = param->xPos;
        if (param->yPos != -1) dlgTemplate->y = param->yPos;
    }
    CWin32InputBox inputbox(param);
    // Resize dialog and SHOW or HIDE multiline
    INT_PTR r = ::DialogBoxIndirectParam(param->hInstance, dlgTemplate, param->hwndOwner,
                                         (DLGPROC)DlgProc, (LPARAM)&inputbox);
    return r;
}

INT_PTR CWin32InputBox::InputBox(char *szTitle, char *szPrompt, char *szResult, DWORD nResultSize,
                                 bool bMultiLine, HWND hwndParent) {
...
void CWin32InputBox::InitDialog() {
    ...
    if (_param->bMultiline) {
        ...
        ::SetWindowPos(_param->hDlg, HWND_NOTOPMOST, 0, 0, rectDlg.right - rectDlg.left,
                       rectDlg.bottom - rectDlg.top - (rectEdit1.bottom - rectEdit1.top),
                       SWP_NOMOVE);
    } else {
        ::SetWindowPos(_param->hDlg, HWND_NOTOPMOST, 0, 0, rectDlg.right - rectDlg.left,
                       rectEdit1.bottom - rectDlg.top + 5, SWP_NOMOVE);
...
LRESULT CALLBACK CWin32InputBox::DlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    switch (message) {
        ...
        case WM_COMMAND: {
            ...
            for (size_t i = 0; i < sizeof(definputbox_buttonids) / sizeof(definputbox_buttonids[0]);
                 i++) {
                if (buttonId == definputbox_buttonids[i]) {
                    ::GetWindowText(_this->_hwndEditCtrl, _this->_param->szResult,
